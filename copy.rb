...
class Shooting
    ...
    @@image_background = Image.load("./bg.png") #クラス変数@@image_backgroundを宣言し、bg.pngファイルを代入
    ...
    def initialize #initializeメソッドが呼び出された際(newメソッドによって)に、メソッド内の処理を行う。
        ...
        @move_ufo = 2    #インスタンス変数@move_ufoを2で初期化、UFOの移動量
    ...
    def fire #弾丸の射出
        if @ufo_HP >= 1    #エンターキーを押したとき(@ufo_HP >= 1が無いと、下のcollisionメソッドで表示した「You　Win!」の文字がエンターキーを押すことで消えてしまう、これは@ufo_HPが0未満になってif @ufo_HP == 0の条件を満たさなくなってしまうからである。)
            if @bullets_y = 420
                Window.draw(@fighter_coo_x-15, @bullets_y, @@image_bullets) #戦闘機の先端(の座標)から弾丸を打ち出す(この段階では表示のみ)
                        @bullets_y -= @move_bullet  #弾丸を移動させる(縦方
                        itinokotei = @fighter_coo_x
            end
            if  (@bullets_y != 420) && (@fighter_coo_x-15+30 >= @ufo_x+30) && (@fighter_coo_x+15 <= @ufo_x+230)    #弾丸のx座標がUFOの横幅よりも内側にある時(数字が足されているのは画像の余白等を考慮した結果)
            
                    Window.draw(itinokotei-15, @bullets_y, @@image_bullets) #戦闘機の先端(の座標)から弾丸を打ち出す(この段階では表示のみ)
                        @bullets_y -= @move_bullet  #弾丸を移動させる(縦方
                    ...
                    if @bullets_y >= 0   #弾丸がウィンドウの上端にあtaっていないとき
                        Window.draw(@fighter_coo_x-15, @bullets_y, @@image_bullets)  #引き続き弾丸を表示  #引き続き弾丸を動かす
                    end   
